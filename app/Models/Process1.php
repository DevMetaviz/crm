...
class Process extends Model
{
    ...
    public function active_parameters()
    {
        return $this->morphMany(Parameter::class, 'parameterable')->where('activeness','like','active')->orderBY('sort_order','asc');
    }
    ...
    public static function process_detail($procees_id)
    {
      $let=Process::find($procees_id);
      $sub_processes=$let->active_sub_processes();
      $parameters=$let->attributes();
    ...
    public function active_sub_processes()
    {
        $stages=$this->sub_stages;
        ...
        foreach ($stages as $key ) {
            ...
            if($key['pivot']['active']==0)
             ...
             $parameters=$key->attributes();
             //$parameters=$key->attributes_with_value();

              $sub_processes=$key->active_sub_processes();
    ...
    public function super_processes()
    {
        return $this->belongsToMany(Process::class,'sub_process','sub_process_id','process_id')->withPivot('sort_order','qc_required','active')->withTimestamps();  
    }
    ...
    {
        return $this->belongsToMany(inventory::class,'product_process','process_id','product_id')->withPivot('sort_order')->withTimestamps();
    }
