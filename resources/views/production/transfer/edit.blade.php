...
<form role="form" id="ticket_form" method="post" action="{{url('transfer-note/update')}}">
          ...
          <div class="col-sm-6">
            ...
            <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                      <li><a href="{{url('print/transfer-note/'.$yield['id'])}}" class="dropdown-item">Print</a></li>
                    </ul>
                  </div>
          </div>
          <div class="col-sm-6">
...
@section('content')
      ...
      <div class="container-fluid" style="margin-top: 10px;">
         ...
         <div class="card">
              <div class="card-header">
                <h3 class="card-title">Edit Transfer Note</h3>
    ...
    <div class="alert alert-success alert-dismissible col-md-3">
      <button type="button" class="close" data-dismiss="alert">&times;</button>
        ...
        <input type="hidden" form="ticket_form" value="{{$yield['id']}}" name="id"/>          
                ...
                <div class="form-group row">
                  ...
                  <input type="number" step="any" form="ticket_form" name="qa_sample" id="qa_sample" class="form-control  col-sm-8" value="{{$yield['qa_sample']}}" required onchange="setLoss()"  style="width: 100%;">
                  </div>
                 </div>
                ...
                <div class="form-group row">
                  ...
                  <input type="number" step="any" form="ticket_form" name="qc_sample" id="qc_sample" class="form-control  col-sm-8" value="{{$yield['qc_sample']}}" required onchange="setLoss()"  style="width: 100%;">
                  </div>
                 </div>
                ...
                <div class="form-group row">
                  ...
                  <input type="number" step="any" form="ticket_form" name="cost_price" id="cost_price" class="form-control  col-sm-8" value="{{$yield['cost_price']}}"  required style="width: 100%;">
                  </div>
                 </div>
                ...
                <div class="form-group row">
                  ...
                  <textarea class="form-control col-sm-10" placeholder="Comment..." form="ticket_form" name="remarks">{{$yield['remarks']}}</textarea>
                  </div>
                 </div>
      ...
      <table class="table table-bordered table-hover "  id="item_table" >
        ...
        <tbody id="selectedItems">
         ...
         <?php $row=1; ?>
             ...
             @foreach($yield['yield_items'] as $item)
          <tr ondblclick="(editItem(<?php echo $row; ?>))" id="{{$row}}">
      
      
    
     <input type="hidden" form="ticket_form" id="{{'tran_dates_'.$row}}" name="tran_date[]" value="{{$item['transfer_date']}}" readonly >
     
     
     <input type="hidden" form="ticket_form" id="{{'qtys_'.$row}}" name="qty[]"  value="{{$item['qty']}}" >
     <input type="hidden" form="ticket_form" id="{{'units_'.$row}}" name="unit[]" value="{{$item['unit']}}" readonly >
     
     
     <input type="hidden" form="ticket_form" id="{{'p_ss_'.$row}}" name="pack_size[]"  value="{{$item['pack_size']}}" >
     
    
     <?php $t=$item['qty'] * $item['pack_size'] ?>
     <td></td>
     <td id="{{'tran_date_'.$row}}">{{$item['transfer_date']}}</td>
    
       <td id="{{'unit_'.$row}}">{{$item['unit']}}</td>
     <td id="{{'qty_'.$row}}">{{$item['qty']}}</td>
    <td id="{{'p_s_'.$row}}">{{$item['pack_size']}}</td>
     <td id="{{'total_'.$row}}">{{$t}}</td>
...
<!-- End Tabs -->
    ...
    <form role="form" id="delete_form" method="POST" action="{{url('transfer-note/delete/'.$yield['id'])}}">
