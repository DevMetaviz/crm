...
<form role="form" id="ticket_form" method="post" action="{{url('save/ticket/stage/table/')}}">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 style="display: inline;">{{$ticket_table['process']['process']['process_name']}}</h1>
      ...
      <div class="container-fluid" style="margin-top: 10px;">
         ...
         <div class="card">
              <div class="card-header">
                <h3 class="card-title">{{'Ticket #: '.$ticket['batch_no'].' Product: '.$ticket['product']['item_name']}}</h3>
      ...
      <button type="button" class="close" data-dismiss="alert">&times;</button>
        {{ session()->get('success') }}
            ...
            <input type="hidden" form="ticket_form" value="{{csrf_token()}}" name="_token"/>
                ...
                <input type="hidden" form="ticket_form" value="{{$ticket_table['id']}}" name="table_id"/>
                ...
                <div class="form-group row">
                  ...
                  <input type="text" form="ticket_form" name="product" class="form-control select2 col-sm-8" value="{{$ticket['product']['item_name']}}" readonly required style="width: 100%;">
                  </div>
                 </div>
                ...
                <div class="form-group row">
                  ...
                  <input type="text" form="ticket_form" name="std_no" class="form-control select2 col-sm-8" value="{{$ticket['batch_no']}}" readonly required style="width: 100%;">
                  </div>
                 </div>
                ...
                <div class="form-group row">
                  ...
                  <input type="text" form="ticket_form" name="process_name" class="form-control select2 col-sm-8" value="{{$ticket_table['process']['process']['process_name']}}" readonly required style="width: 100%;">
                  </div>
                 </div>
                         ...
                         $count=count($ticket_table['columns']);

                         $row_count=$ticket_table->default_row_count();
                        ...
                        <table class="table table-bordered table-hover">
                          ...
                          <thead>
                             <tr>
                                ...
                                @for($i=0;$i<$count;$i++)
                                  <th>{{$ticket_table['columns'][$i]['column']['heading']}}</th>
                                @endfor
                                <th></th>
                             </tr>
                            ...
                            @for($j=1;$j<=$row_count;$j++)
                             <tr id="{{'row_'.$row}}">
                               @for($i=0;$i<$count;$i++)
                                  ...
                                  $type=$ticket_table['columns'][$i]['column']['type'];
                                        $head=$ticket_table['columns'][$i]['column']['heading'];
                                  $value=$ticket_table['columns'][$i]->row_value($ticket_table['id'],$ticket_table['columns'][$i]['id'],$j); 
                              ...
                              <td>
                                ...
                                $col='col_'.$ticket_table['columns'][$i]['id'].'[]';
                                  ?>
                             ...
                             <tr>
                               @foreach($ticket_table['columns'] as $cl)
                                        <?php 
                                              if($cl['column']['footer_type']=='sum')
                                              {
                                              $sum=$cl->col_sum($ticket_table['id']);
                                              }
                                         ?>
                               <th>
                                @if($cl['column']['footer_type']=='sum')
...
function setRowNum()
{
  ...
  function addRow()
  {
   ...
   var row=getRowNum();
     ...
     var cols=`<?php echo json_encode($ticket_table['columns']); ?>`; 
