...
@section('content-header')
    ...
    <form role="form" id="purchase_demand" method="POST" action="{{url('update/point/sale')}}" onkeydown="return event.key != 'Enter';" >
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 style="display: inline;">Edit Point Sale</h1>
            ...
            <a class="btn" href="{{url('point/sale/history')}}" style="border: none;background-color: transparent;"><span class="fas fa-edit">&nbsp</span>History</a>
            <a class="btn" href="{{url('point/sale/create')}}" style="border: none;background-color: transparent;"><span class="fas fa-edit">&nbsp</span>New</a>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item"><a href="#">Point Sale</a></li>
  ...
  .alert-inline{
    ...
    <div class="alert alert-success alert-dismissible">
      ...
      <div id="error" style="display: none;"><p class="text-danger" id="error_txt"></p></div>
            ...
            <div class="row">
              <div class="col-md-4">
                ...
                <div class="form-group row">
                  ...
                  <input type="text" form="purchase_demand" name="doc_no" class="form-control  col-sm-8" value="{{$sale['doc_no']}}"  required style="width: 100%;">
                  </div>
                ...
                <div class="form-group row">
                  ...
                  <input type="date" form="purchase_demand" name="doc_date" class="form-control col-sm-8" value="{{$sale['doc_date']}}" required style="width: 100%;">
                  </div>

                  <div class="form-group row">
                  <label class="col-sm-4">Invoice No.</label>
                  <input type="text" form="purchase_demand" name="invoice_no" class="form-control  col-sm-8" value="{{$sale['invoice_no']}}"  style="width: 100%;">
                  </div>
           ...
           <div class="form-group">
                  <label>Point</label>
                  <select form="purchase_demand" name="point_id" id="point_id" class="form-control select2" onchange="setDistributor()" required>
                    <option value="">Select any point</option>
                    @foreach($points as $pt)
                    <?php 

                       $s='';
                         if($pt['id']==$sale['point_id'])
                          $s='selected';
                     ?>
                     <option value="{{$pt['id']}}" {{$s}} >{{$pt['name']}}</option>
                     @endforeach
                  </select>
                  </div>
                  ...
                  <select form="purchase_demand" name="distributor_id" id="distributor_id" class="form-control select2" onchange="" required>
                     <option value="">Select any distributor</option>
                     @foreach($customers as $pt)
                     <?php 

                       $s='';
                         if($pt['id']==$sale['distributor_id'])
                          $s='selected';
                     ?>
                     <option value="{{$pt['id']}}" {{$s}} >{{$pt['name']}}</option>
                     @endforeach
                  ...
                  <select class="form-control select2" name="salesman_id" id="salesman_id"   style="width: 100%;">
                    ...
                    <?php 
                       ...
                       $s='';
                         if($cus['id']==$sale['salesman_id'])
                          $s='selected';
                     ?>
                    <option value="{{$cus['id']}}" {{$s}}>{{$cus['name']}}</option>
                    @endforeach
                  ...
                  <select class="form-control" name="type" id="type"   style="width: 100%;">
                    <?php 
                       ...
                       $s=''; $s1='';
                         if($sale['type']=='invoice_wise')
                          $s='selected';
                        elseif($sale['type']=='item_wise')
                          $s1='selected';

                     ?>
                    <option value="invoice_wise" {{$s}}>Invoice Wise</option>
                    <option value="item_wise" {{$s1}}>Item Wise</option>
                  </select>
                </div>

                   <div class="form-group">
                  <label>Sale Value</label>
                  <input type="number" step="any" name="sale_value" value="{{$sale['sale_value']}}" form="purchase_demand" class="form-control">
                </div>
                   ...
                   <div class="form-group">
                  <label>Remarks</label>
                  <textarea name="remarks" form="purchase_demand" class="form-control" >{{$sale['remarks']}}</textarea>
                </div>
...
<div class="tab-content" style="background-color: white;border: 1px solid #dee2e6;padding: 10px;min-height:200px;">
    ...
    <div class="tab-pane fade show active" id="tabA">
     ...
     <td><input type="number" value="{{$item['pivot']['qty']}}" min="1" form="purchase_demand" name="qtys[]" id="{{'qty_'.$row}}" onchange="rate_change('{{$row}}')" required="true"></td>

     <td><input type="number" value="{{$item['pivot']['rate']}}" min="0" form="purchase_demand" name="rates[]" id="{{'rate_'.$row}}" onchange="rate_change('{{$row}}')"  required="true"></td>

     <?php $total=$item['pivot']['qty'] * $item['pivot']['rate']; ?>
     <td><input type="number" value="{{$total}}" min="0" form="purchase_demand" name="totals[]" id="{{'total_'.$row}}" onchange="set_rate('{{$row}}')"  required="true"></td>
...
function addItem()
{
  ...
  if(item_id!='')
  {
     ...
     if(item_id=='' ||  rate=='' || qty=='' || dbl_item==true)
     {
     ...
     <td><input type="number" value="${qty}" min="1" form="purchase_demand" name="qtys[]" id="qty_${row}" onchange="rate_change(${row})" required="true"></td>

     <td><input type="number" value="${rate}" min="0" form="purchase_demand" name="rates[]" id="rate_${row}" onchange="rate_change(${row})"  required="true"></td>
...
function setDistributor()
{
  ...
  var points=JSON.parse( '<?php echo json_encode(  $points) ; ?>' );
  
   let point = points.findIndex((item) => item.id == point_id);
    var distributor_id=points[point]['distributor_id'];
    
    $('#distributor_id').val(distributor_id);
    $('#distributor_id').trigger('change');

     var salesman_id=points[point]['salesman_id'];
    
    $('#salesman_id').val(salesman_id);
    $('#salesman_id').trigger('change');

    var doctor_id=points[point]['doctor_id'];
