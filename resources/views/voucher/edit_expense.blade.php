    ...
    <!-- Content Header (Page header) -->
       ...
       <div class="row default-header"  >
          ...
          <div class="col-sm-6 text-right">
            ...
            <a class="btn btn-transparent" href="{{url('expense/create')}}" ><span class="fas fa-plus">&nbsp</span>New</a>
            <a class="btn btn-transparent" href="{{url('expense/history')}}" ><span class="fas fa-history">&nbsp</span>Expense List</a>
             ...
             <div class="btn-group">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                       <li><a href="{{url('/voucher/report/'.$payment['id'])}}" class="dropdown-item">Voucher</a></li>
                      <li><a href="{{url('/voucher/report1/'.$payment['id'])}}" class="dropdown-item">Voucher1</a></li>
                      <li><a href="{{url('/voucher/report2/'.$payment['id'])}}" class="dropdown-item">Voucher2</a></li>
...
@section('content')
    <!-- Main content -->
     ...
     <form role="form" id="purchase_demand" method="POST" action="{{url('/expense/update')}}">
      ...
      <div id="std_error" style="display: none;"><p class="text-danger" id="std_error_txt"></p></div>
            ...
            <div class="row">
              <div class="col-md-9">
                   ...
                   <h4 class="form-section-title"><i class="fas fa-file-alt mr-2"></i>Expense Voucher</h4>
                ...
                <div class="col-md-3 form-group ">
                  ...
                  <input type="text" form="purchase_demand" name="voucher_no" id="voucher_no" class="form-control" value="{{$payment['voucher_no']}}" readonly required style="width: 100%;">
                  </div>


                <div class="col-md-3 form-group">
                  <label>Voucher Date</label>
                  <input type="date" form="purchase_demand" name="voucher_date" class="form-control" value="{{$payment['voucher_date']}}" id="voucher_date" onchange="setDocNo()" required style="width: 100%;">
                  </div>

                  <div class="col-md-2 form-group">
                  <label>Payment Mehtod</label>
                  <select form="purchase_demand" name="pay_method" id="pay_method" class="form-control " onchange="setDocNo()" required>
                  ...
                  <select form="purchase_demand" name="pay_from" id="pay_from" class="form-control " required>
                  ...
                  <input type="text" form="purchase_demand" name="reference" class="form-control" value="{{$payment['reference']}}"  style="width: 100%;">
                  </div>

                  <div class="col-md-2 form-group">
                  <!--<input type="checkbox" form="purchase_demand" name="activeness" value="1" id="activeness" class=""  >-->
                  <label>Status</label>
                  <select form="purchase_demand" name="status" class="form-control" required >
                        <option value="1" @if($payment['status']==1){{'selected'}}@endif >Post</option>
                        <option value="0" @if($payment['status']==0){{'selected'}}@endif >Unpost</option>
                    </select>
              ...
              <div class="col-md-9">
                   ...
                   <div class="form-section">
                   <h4 class="form-section-title"><i class="fas fa-plus-circle mr-2"></i>Expense Detail</h4>
              ...
              <div class="dropdown col-sm-9" id="accounts_table_account_dropdown">
                ...
                <input class="form-control" form="add_item"  name="account_id" id="account_id" required>
                  ...
                  <label>Amount</label>
                  <input type="number" value="0" min="0" step="any" form="add_item" name="amount" id="amount" class="form-control" style="width: 100%;">
                  </div>

                   <div class="form-group col-sm-12">
                  <label>Remarks</label>
                  <textarea name="remarks" id="remarks" form="add_item" class="form-control" ></textarea>
                  </div>
    ...
    <div class="tab-pane fade show active" id="tabA">
      ...
      <table class="table table-bordered table-hover"  id="item_table" >
        ...
        <tbody id="selectedItems">
          ...
          <input type="hidden" form="purchase_demand" id="{{'credit_pivot_id'}}" name="credit_pivot_id" value="{{$payment['credit_transection']['pivot']['id']}}" readonly >

          <?php $row=1; ?>
            @foreach($payment['debit_transections'] as $acc)
              <tr id="{{$row}}" ondblclick="editItem('{{$row}}')">
              <th></th>

               <input type="hidden" form="purchase_demand" id="{{'pivot_id_'.$row}}" name="pivots_id[]" value="{{$acc['pivot']['id']}}" readonly >
     
           <input type="hidden" form="purchase_demand" id="{{'account_id_'.$row}}" name="accounts_id[]" value="{{$acc['id']}}" readonly >
 
    
     <td id="{{'account_name_'.$row}}">{{$acc['name']}}</td>
     <td id="{{'account_code_'.$row}}">{{$acc['code']}}</td>
       
  

     <td><input type="text" class="form-control" value="{{$acc['pivot']['remarks']}}" form="purchase_demand" name="remarks[]" id="{{'remarks_'.$row}}" readonly ></td>
     <td><input type="text" class="form-control" value="{{$acc['pivot']['cheque_no']}}" form="purchase_demand" name="cheque_no[]" id="{{'cheque_no_'.$row}}" readonly ></td>
<td><input type="text" class="form-control" value="{{$acc['pivot']['cheque_date']}}" form="purchase_demand" name="cheque_date[]" id="{{'cheque_date_'.$row}}" readonly ></td>

     <td><input type="text" class="form-control" value="{{$acc['pivot']['debit']}}" form="purchase_demand" name="amount[]" id="{{'amount_'.$row}}" readonly ></td>
...
<!-- End Tabs -->
    ...
    <form role="form" id="delete_form" method="POST" action="{{url('/delete/expense/'.$payment['id'])}}">
     @csrf    
    </form>
...
$(document).ready(function(){
  ...
  type="{{ $payment['pay_method'] }}";
     $('#pay_method').val(type);

  type="{{ $payment['pay_from'] }}";
     $('#pay_from').val(type);
