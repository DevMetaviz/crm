...
$('.multiImgFrm').validate({
    ...
    submitHandler: function (form) {
        ...
        // Make sure this points to your form
        ...
        selectedFiles.forEach(file => {
            formData.append('proofs[]', file.file ? file.file : file); // handle file object
            console.log(file); // debug each file
        });

        // AJAX request
        $.ajax({
            url: $(form).attr('action'), // make sure 'action' attribute exists in form
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(res) {
                ...
                selectedFiles = [];
                renderPreview(); // reset previews
                form.reset(); // reset other form fields
            },
            error: function(err) {
                console.error(err.responseText || err); // show Laravel error
