      ...
      <div class="row default-header"  >
          ...
          <div class="col-sm-6 text-right">
             ...
             @if(isset($order['id']))
                        ...
                        <a href="{{url('purchase/order')}}" class="btn btn-action">
...
<form role="form" id="purchase_demand" method="POST" action="{{ isset($order['id']) ? url('/purchase/order/update/') : url('/purchase/order/save') }}">
      ...
      <div id="std_error" style="display: none;"><p class="text-danger" id="std_error_txt"></p></div>
            ...
            <div class="row">
                ...
                <div class="form-group col-md-4">
                  ...
                  <label>Due Date</label>
                   <input type="date" form="purchase_demand" name="due_date" class="form-control " value="{{ old('due_date', $order->due_date) }}"   >
                  </div>
        ...
        @endforeach
            ...
            <option value="{{ $book->id }}"
                {{ old('booking_id', $order->booking_id ?? null) == $book->id ? 'selected' : '' }}>
                {{ $book->doc_no }}
            </option>
        @endforeach
...
<div class="row mt-2">
  ...
  <div class="col-md-6">
         ...
         <?php 
                 //$per_freight=1.5; $per_loading=0.5; $per_unloading=0.15; //echo $total_weight.' '.$order['freight_charges'];

                  $per_freight=0; $per_loading=0; $per_unloading=0;
         ...
         ?>
          ...
          <div class="form-group row">
            <label  class="col-sm-4 col-form-label">Unloading Charges</label>
            <div class="col-sm-4">
...
let bookings = @json($bookings);
  ...
  let booking_categories = @json($order->booking->categories ?? []);
...
$('.select2').select2(); 
  ...
  function setBookings(books){
        
        bookings=books; 
        
  }

  function setBookingCategories(categories){
...
$('select[name="booking_id"]').on('change', function () {
        ...
        if(booking_id){
              ...
              let selectedBooking = bookings.find(booking => booking.id == booking_id);
                 
           if(selectedBooking && selectedBooking.categories) {
            setBookingCategories(selectedBooking.categories);  
           } else {
...
function getVendorBookings() {
        ...
        setBookings([]);   setBookingCategories([]);

        let itemSelect = $('select[name="booking_id"]');
        ...
        if (vendor_id) {
            ...
            $.ajax({
                url: '{{ url('get-vendor-pending-bookings') }}',
                type: 'GET',
                data: {
                    vendor_id: vendor_id
                },
                        ...
                        bookings=response.data;
                          ...
                          setBookings(bookings);  
                             ...
                             bookings.forEach(item => {
                              itemSelect.append(
                                  $('<option>', { value: item.id, text: item.doc_no })
                               );
                            });
...
$(document).on('change', 'select[name="category_id[]"]', function () {
    ...
    if (categoryId) {
        ...
        filteredItems.forEach(item => {
            ...
            let cat = booking_categories.find(it => parseInt(it.id) === parseInt(categoryId));

                    
            if (cat && cat.pivot) {
            row.find('input[name="disc[]"]').val(cat.pivot.discount ?? '');
            }
            else{
                row.find('input[name="disc[]"]').val('');
            }
