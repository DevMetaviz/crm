...
@section('title')
    {{ isset($order['id']) ? 'Edit the GRN' : 'Add New GRN'}}
@endsection
@section('header-css')
    ...
    <!-- Content Header (Page header) -->
      ...
      <div class="row default-header"  >
          ...
          <div class="col-sm-6 text-right">
             ...
             <button form="purchase_demand" type="submit" class="btn btn-primary"><span class="fas fa-save">&nbsp</span>{{ isset($order['id']) ? 'Update' : 'Save' }}</button>

              @if(isset($order['id']))
                        <button type="button" class="btn btn-action" data-toggle="modal" data-target="#modal-del" >
                            <i class="fas fa-trash"></i> Delete
                        </button>
                        <a href="{{url('/purchase/goods-receiving-note')}}" class="btn btn-action">
           ...
           @if(isset($order['id']))
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                      <li><a href="{{url('/grn/inward-gatepass/'.$order['doc_no'])}}" class="dropdown-item">Inward Gatepass</a></li>
                    </ul>
                  </div>
           ...
           <ol class="breadcrumb default-breadcrumb"  >
              ...
              <li class="breadcrumb-item"><a href="#">Goods Received Note</a></li>
...
<form role="form" id="delete_form" method="POST" action="{{url('purchase/grn/delete/'.$order['id'])}}">
               
               @csrf    
    </form>

<form role="form" id="purchase_demand" method="POST" action="{{ isset($order['id']) ? url('update/purchase/grn') : url('purchase/grn/save') }}">
      ...
      <div id="std_error" style="display: none;"><p class="text-danger" id="std_error_txt"></p></div>
            ...
            <div class="row">
                ...
                <div class="form-group col-md-4">
                  ...
                  <div class="form-group col-md-4">
                  <label>PO No.</label>
                  <input type="text" form="purchase_demand" name="order_no" class="form-control" value="{{ $order->order_no ?? $order->order->doc_no ?? '' }}" readonly >
                ...
                <div class="form-group col-md-4">
                  <label>PO Date</label>
                  <input type="date" form="purchase_demand" name="order_date" class="form-control" value="{{  $order->order_date ?? $order->order->doc_date ?? '' }}" readonly >
                  </div>
      ...
      <table class="table table-hover table-head-fixed text-nowrap mb-0 add-lines-table"  id="item_table" >
        ...
        <?php 
             ...
             $av_qty_txt='';  $av_qty='';  $av_rack_qty='';
                   ...
                   if($info['rack_qty']!='')
                    {
                        //$av_qty_txt .=' ['.$info['rack_qty'].']';  
                           //$av_rack_qty=' ['.$info['rack_qty'].']';
                            }
                }
        ...
        <tr class="item-row">
            ...
            <!--<td class="loading_sq">
                <input type="text" step="any" value="{{ $item->pivot->loading_sq }}" form="purchase_demand" name="loading_sq[]" class="form-control" >
            </td>-->
    ...
    @else
      ...
      <td class="total_feet"></td>
       ...
       <td class="rack">
            ...
            <!---<td class="loading_sq">
                <input type="text" step="any" value="" form="purchase_demand" name="loading_sq[]" class="form-control" >
            </td>-->
...
function updateChargesAndTaxes(totalWeight, baseAmount) {
    ...
    /*// --- Taxes ---
    let tax1 = parseFormattedNumber($("input[name='tax1']").val()) || 0;
    let tax2 = parseFormattedNumber($("input[name='tax2']").val()) || 0;
    let tax3 = parseFormattedNumber($("input[name='tax3']").val()) || 0;
