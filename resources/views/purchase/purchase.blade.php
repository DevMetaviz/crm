...
@section('title')
    {{ isset($order['id']) ? 'Edit the Invoice' : 'Add New Invoice' }}
@endsection
@section('header-css')
      ...
      <div class="row default-header"  >
          ...
          <div class="col-sm-6 text-right">
             ...
             @if(isset($order['id']))
                        ...
                        <a href="{{url('purchase/')}}" class="btn btn-action">
            ...
            @if(isset($order['id']))
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                      <li><a href="{{url('/purchase/report/'.$order['id'])}}" class="dropdown-item">Print</a></li>
                    </ul>
                  </div> 
                  @endif
...
<form role="form" id="delete_form" method="POST" action="{{url('purchase/delete/'.$order['id'])}}">
               
               @csrf    
    </form>


<form role="form" id="purchase_demand" method="POST" action="{{ isset($order['id']) ? url('update/purchase/') : url('/purchase/save') }}">
            ...
            <div class="row">
                ...
                <div class="form-group col-md-4">
                  ...
                  <label>Invoice Due Date</label>
                   <input type="date" form="purchase_demand" name="due_date" class="form-control " value="{{ old('due_date', $order->due_date) }}"  required >
                  </div>
                ...
                <option value="cash" {{ old('invoice_type', $order->invoice_type) == 'cash' ? 'selected' : '' }}>Cash</option>
                 <option value="credit" {{ old('invoice_type', $order->invoice_type) == 'credit' ? 'selected' : '' }}>Credit</option>
                   ...
                   <div class="form-group col-md-4">
                  <label>GRN No.</label>
                  <input type="text" form="purchase_demand" name="grn_no" class="form-control" value="{{ $order->grn_no ?? $order->grn->doc_no ?? '' }}" readonly >
                  <input type="hidden" form="purchase_demand" name="grn_id" class="form-control" value="{{ $order->grn_id }}" readonly >
                  </div>


                <div class="form-group col-md-4">
                  <label>GRN Date</label>
                  <input type="date" form="purchase_demand" name="grn_date" class="form-control" value="{{  $order->grn_date ?? $order->grn->doc_date ?? '' }}" readonly >
                  </div>
...
<div class="tab-content" style="">
    ...
    <div class="tab-pane fade show active" id="tabA">
      ...
      <table class="table table-hover table-head-fixed text-nowrap mb-0 add-lines-table"  id="item_table" >
        ...
        <tr class="item-row">
            ...
            <td>
                <input type="number" step="any" value="{{ $item->pivot->unit_weight }}" form="purchase_demand" name="unit_weight[]" class="form-control last-input" required>
    ...
    @else
      ...
      <td class="rate"></td>
      <td>
        <input type="number" step="any" value="1" form="purchase_demand" name="unit_weight[]" class="form-control last-input" required>
...
<?php 
                 $per_freight=1.5; $per_loading=0.5; $per_unloading=0.13; 
                  $per_freight=0; $per_loading=0; 

                    if(isset($order['freight_charges']) )
                    {
                     $per_freight= round($order['freight_charges']/$total_weight,2); 
                      $per_loading= round($order['loading_charges']/$total_weight,2); 
                      $per_unloading= round($order['unloading_charges']/$total_weight,2); 
                   }
...
<div class="row mt-2">
  ...
  <div class="col-md-5">
    ...
    <div class="form-group row">
            <label  class="col-sm-4 col-form-label-1">
                Unloading Charges:
            </label>
            <div class="col-sm-4">
              <input type="number" step="any" name="unloading_charges_per_unit" class="form-control"  value="{{$per_unloading}}" >
            </div>
            <div class="col-sm-4">
              <input type="number" step="any" name="unloading_charges" class="form-control"  value="{{ old('unloading_charges', $order->unloading_charges) }}" >
            </div>
          </div>
          ...
          <div class="form-group row">
            <label  class="col-sm-4 col-form-label-">
                <select class="form-control" name="freight_expense_payable_id" >
                    <option value="" >Select any value</option>
                    @foreach($freight_expenses_payables as $fr)
                    <option value="{{$fr['id']}}" {{ $fr->id == $order->freight_expense_payable_id ? 'selected' : '' }} >{{$fr['name'].' '.$fr['code']}}</option>
...
<h4 class="bold text-center">Purchase Voucher Transactions</h4>               
...
@foreach($transactions as $accountId => $accountTrans)
    <?php
       ...
       if(isset($accountTrans[0]['account']['code']))
    ...
    <tr>
        ...
        <td>{{ number_format($debitSum,2) }}</td>
        <td>{{ number_format($creditSum,2) }}</td>
    </tr>
    <?php  $i++; ?>
@endforeach
</tbody>
...
@if(isset($order['freight_voucher']))
<?php 
...
$transactions = collect($order['freight_voucher']['transections'])->groupBy('account.id'); 

?>

<h4 class="bold text-center">Freight JV</h4>               
...
function updateChargesAndTaxes(totalWeight, baseAmount) {
    ...
    let $unloadingPerUnitInput = $("input[name='unloading_charges_per_unit']");
    let $unloadingTotalInput = $("input[name='unloading_charges']");
    ...
    let unloadingPerUnit = parseFormattedNumber($unloadingPerUnitInput.val()) || 0;
    let unloadingTotal = parseFormattedNumber($unloadingTotalInput.val()) || 0;
    ...
    // --- Unloading Logic ---
    if ($unloadingPerUnitInput.is(":focus")) {
        unloadingTotal = unloadingPerUnit * totalWeight;
        $unloadingTotalInput.val(unloadingTotal ? unloadingTotal.toFixed(2) : "");
    } else if ($unloadingTotalInput.is(":focus")) {
        unloadingPerUnit = totalWeight > 0 ? (unloadingTotal / totalWeight) : 0;
        $unloadingPerUnitInput.val(unloadingPerUnit ? unloadingPerUnit.toFixed(2) : "");
    } else {
        unloadingTotal = unloadingPerUnit * totalWeight;
        $unloadingTotalInput.val(unloadingTotal ? unloadingTotal.toFixed(2) : "");
    }
    ...
    // --- Final Total ---
    let grandTotal = baseAmount  + loadingTotal ;  //+ tax1Amt + tax2Amt + tax3Amt;
    $("input[name='total_amount']").val(formatNumber(grandTotal,2));
}


$(document).on("input", "input[name='freight_charges_per_unit'], input[name='freight_charges'], input[name='loading_charges_per_unit'], input[name='loading_charges'], input[name='unloading_charges_per_unit'], input[name='unloading_charges'], input[name='tax1'], input[name='tax2'], input[name='tax3']", function () {
    updateFooterTotals();
});
