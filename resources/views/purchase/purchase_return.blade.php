...
@section('title')
    {{ isset($order['id']) ? 'Edit the Purchase Return' : 'Add New Purchase Return' }}
@endsection
@section('header-css')
      ...
      <div class="row default-header"  >
          ...
          <div class="col-sm-6 text-right">
             ...
             @if(isset($order['id']))
                        ...
                        <a href="{{url('purchase/return/')}}" class="btn btn-action">
                            <i class="fas fa-plus"></i> New
                        </a>
                        @endif
            
            <a class="btn btn-transparent" href="{{url('purchase/return/history')}}" ><span class="fas fa-history"></span>History</a>
            ...
            @if(isset($order['id']))
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                      <li><a href="{{url('/purchase/return/report/'.$order['id'])}}" class="dropdown-item">Print</a></li>
                    </ul>
                  </div> 
                  @endif
...
<form role="form" id="delete_form" method="POST" action="{{url('purchase/return/delete/'.$order['id'])}}">
               
               @csrf    
    </form>


<form role="form" id="purchase_demand" method="POST" action="{{ isset($order['id']) ? url('update/purchase/return') : url('purchase/return/save') }}">
      ...
      <div id="std_error" style="display: none;"><p class="text-danger" id="std_error_txt"></p></div>
            ...
            <div class="row">
                ...
                <div class="form-group col-md-4">
                  ...
                  <input type="text" form="purchase_demand" name="purchase_no" class="form-control" value="{{ $order->purchase_no ?? $order->purchase->doc_no ?? '' }}" readonly >
                  <input type="hidden" form="purchase_demand" name="purchase_id" class="form-control" value="{{ $order->purchase_id }}" readonly >
                  </div>


                <div class="form-group col-md-4">
                  <label>Purchase Date</label>
                  <input type="date" form="purchase_demand" name="purchase_date" class="form-control" value="{{  $order->purchase_date ?? $order->purchase->doc_date ?? '' }}" readonly >
                  </div>
        ...
        @endforeach
                   ...
                   <div class="form-group col-md-4">
                  <label>Remarks</label>
                  <input name="remarks" form="purchase_demand" class="form-control" value="{{ old('remarks', $order->remarks) }}" >
                </div>
...
<div class="row mt-2">
  ...
  <div class="col-md-6">
         ...
         <?php 
                 //$per_freight=1.5; $per_loading=0.5; 
         ...
         ?>
          ...
          <div class="form-group row">
            <label  class="col-sm-4 col-form-label-1">
                Loading Charges:
                <select class="form-control" name="loading_charges_id" >
                      <option value="" >Select any value</option>
                    @foreach($freight_expenses as $fr)
                    <option value="{{$fr['id']}}" {{ $fr->id == $order->loading_charges_id ? 'selected' : '' }} >{{$fr['name'].' '.$fr['code']}}</option>

                    @endforeach
                </select>

                Loading Charges payable:
                <select class="form-control" name="loading_charges_payable_id" >
                    <option value="" >Select any value</option>
                    @foreach($freight_expenses_payables as $fr)
                    <option value="{{$fr['id']}}" {{ $fr->id == $order->loading_charges_payable_id ? 'selected' : '' }} >{{$fr['name'].' '.$fr['code']}}</option>
