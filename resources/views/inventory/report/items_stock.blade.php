<html>
    ...
    <table width="100%" style="padding: 10px;" cellspacing="0">
        ...
        <tr  style="background-color: #03a9f4;"><td colspan="3"><p class="top-heading">Stock Report</p></td></tr>
         
        <tr class="">
            <td align="" style="width: 50%;">
               
                 
                <p style="text-transform: capitalize;" ><b>Department: </b>{{$depart}}</p>
            </td>
            <?php
                    if($f!='')
                    {
                    $f=date_create($f );
                     $f=date_format($f,"d-M-Y");
                     }
                     
                     if($t!='')
                    {
                     $t=date_create($t );
                     $t=date_format($t,"d-M-Y");
                     }
            ?>
    ...
    <div style="height:30px;background-color: #03a9f4;"></div>
     ...
     <thead style="background-color:#03a9f4;">
        <tr>
            ...
            <th class="col1 col-wt-2">Department</th>
            <th class="item-name-th">Item name</th>
            <th class="col1 col-wt-2">Opening</th>
            
            
            <th class="col1 col-wt-3">Purchase Qty</th>
            <th class="col1 col-wt-3">Production Qty</th>
            <th class="col1 col-wt-3">Issue Qty</th>
            <th class="col1 col-wt-3">Sale Qty</th>
                    
           <th class="col1 col-wt-3">Closing</th>
     ...
     @foreach($items as $item)
        ...
        <tr>
         ...
         <td class="col bottom" style="text-transform: capitalize;">{{$item['location']}}</td>
    
         
         <td class="item-name-col bottom">{{$item['item_name']}}</td>
         <td class="col bottom">{{$item['opening']}}</td>
          <td class="col bottom">{{$item['detail']['grn_qty']}}</td>
          <td class="col bottom">{{$item['detail']['production_qty']}}</td>
          <td class="col bottom">{{$item['detail']['issue_qty']}}</td>
          <td class="col bottom">{{$item['detail']['dc_qty']}}</td>
          <td class="col bottom">{{$item['closing']}}</td>
