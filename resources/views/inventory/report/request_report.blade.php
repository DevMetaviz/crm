<html>
    <head>
        <title>{{$request['requisition_no']}}</title>
        <style>
            /** 
    ...
    <body>
        ...
        <tr style="background-color:#03a9f4;color: white;"><td style="padding: 10px;" class="top-heading" colspan="3"><strong>Requisition</strong></td></tr>
         
        <tr class="">

            <td align=""  style="width: 40%;">
             <?php 
         $d_date=date_create($request['requisition_date'] );
           $d_date=date_format($d_date,"d-M-Y");

          ?>
               <table class="challan-detail" style="width: 80%;">
                 <tr><th>Requisition No:</th><td>{{$request['requisition_no']}}</td></tr>
                 <tr><th>Requisition Date:</th><td>{{$d_date}}</td></tr>
                 <tr><th>Department:</th><td style="text-transform: capitalize;">{{$request['department']['name']}}</td></tr>
               </table>
                
            </td>

            
            <td align="" style="width: 45%">

                <table class="challan-detail" style="width: 80%;">
                    <tr><th>Plan No:</th><td>@if($request['plan']!=''){{$request['plan']['plan_no']}}@endif</td></tr>
                 <tr><th>Product:</th><td>@if($request['plan']!=''){{$request['plan']['product']['item_name']}}@endif</td></tr>
                 <tr><th>Batch No:</th><td>@if($request['plan']!=''){{$request['plan']['batch_no']}}@endif</td></tr>
                 <tr><th>Batch Size:</th><td style="text-transform: capitalize;">@if($request['plan']!=''){{$request['plan']['batch_size']}}@endif</td></tr>
               </table>
    ...
    <div style="height:30px;background-color: #03a9f4;"></div>
     ...
     <thead style="background-color:#03a9f4;color: white;">
        <tr>
            ...
            <th class="col1">Approved Qty</th>
        </tr>
        </thead>
        <?php  $i=1; $total=0; $total1=0;  ?>
     @foreach($request['item_list'] as $item)
     <?php 
          
          $t_qty=$item['quantity']*$item['pack_size']; $t_app_qty=$item['approved_qty']*$item['pack_size'];

           $total += $t_qty; $total1 += $t_app_qty;
          ?>
   ...
   <table class="" style="border-spacing:50px; width: 100%;">
      <tr >
         <th class="sign"><span>Request By</span></th>
