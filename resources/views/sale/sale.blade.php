...
@section('title')
    {{ isset($order['id']) ? 'Edit the Sale Invoice' : 'Add New Sale Invoice' }}
@endsection
@section('header-css')
        ...
        <div class="page-header">
            <div class="row align-items-center">
                ...
                <div class="col-md-6">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                      <li><a href="{{url('/estimated-invoice/report/'.$order['id'].'/invoice')}}" class="dropdown-item">Invoice</a></li>
                      <!--<li><a href="{{url('/export-invoice/report/'.$order['id'])}}" class="dropdown-item">Invoice (Export)</a></li>
                      <li><a href="{{url('/estimated-invoice/report/'.$order['id'].'/tp-invoice')}}" class="dropdown-item">TP Invoice</a></li>
                      <li><a href="{{url('/estimated-invoice/report/'.$order['id'].'/mrp-invoice')}}" class="dropdown-item">MRP Invoice</a></li>-->
                    </ul>
                  </div>
                  @endif
            ...
            <nav aria-label="breadcrumb" class="mt-2">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active text-white" aria-current="page">{{ isset($order['id']) ? 'Edit the Sale Invoice' : 'Add New Sale Invoice' }}</li>
                </ol>
            </nav>
        </div>
      </div>
...
<form role="form" id="purchase_demand" method="POST" action="{{ isset($order['id']) ? url('/sale/update/') : url('/sale/save') }}">
      ...
      <div id="std_error" style="display: none;"><p class="text-danger" id="std_error_txt"></p></div>
            ...
            <div class="row">
                ...
                <div class="form-group col-md-4">
                  ...
                  <input type="text" form="purchase_demand" name="challan_no" class="form-control" value="{{ old('challan_no', $order->challan->doc_no ?? '') }}"  readonly> 
                   <input type="hidden" form="purchase_demand" name="challan_id" class="form-control" value="{{ old('challan_id', $order->challan_id) }}"  > 
                  </div>
...
<div class="tab-content" style="">
      ...
      <table class="table table-hover table-head-fixed text-nowrap mb-0 add-lines-table"  id="item_table" >
        ...
        <tr class="item-row">
            ...
            <td>  <input type="hidden" form="purchase_demand"  name="pivots_id[]" value="{{$item['pivot']['id']}}"  >
                <input type="hidden" form="purchase_demand"  name="av_qty[]" value="{{$item['pivot']['av_qty']}}"  >
            ...
            <td class="qty_av">{{$item['pivot']['av_qty']}}</td>
    ...
    @else
      ...
      <td>
        ...
        <input type="hidden" form="purchase_demand"  name="av_qty[]" value=""  >
        <select class="form-control select2" form="purchase_demand" name="category_id[]" required>
          <option value="">Select any value</option>
          @foreach($categories as $depart)
...
<!-----Transactions---->
    ...
    <form role="form" id="#add_item">
            ...
            <form role="form" id="delete_form" method="POST" action="{{url('/sale/delete/'.$order['id'])}}">
