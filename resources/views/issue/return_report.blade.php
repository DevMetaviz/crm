<html>
    ...
    <body>
        ...
        <tr style="background-color:#03a9f4;color: white;"><td style="padding: 10px;" class="top-heading" colspan="3"><strong>Issuance Return</strong></td></tr>
         
        <tr class="">

            <td align=""  style="width: 50%;">
             <?php 
         $d_date=date_create($return['doc_date'] );
           $d_date=date_format($d_date,"d-M-Y");

          ?>
               <table class="challan-detail" style="width: 80%;">
                 <tr><th>Return No:</th><td>{{$return['doc_no']}}</td></tr>
                 <tr><th>Return Date:</th><td>{{$d_date}}</td></tr>
                 <tr><th>Department:</th><td style="text-transform: capitalize;">{{$return['department']['name']}}</td></tr>
               </table>
            ...
            <td align="" style="width: 25%">
                 @if($return['plan'])
                <table class="challan-detail" style="width: 80%;">
                 <tr><th>Plan No:</th><td>{{$return['plan']['plan_no']}}</td></tr>
                 <tr><th>Batch No:</th><td>{{$return['plan']['product']['item_name']}}</</td></tr>
                 <tr><th>Batch No:</th><td>{{$return['plan']['batch_no']}}</</td></tr>
                 <tr><th>Batch Size:</th><td>{{$return['plan']['batch_size']}}</</td></tr>
                </table>
                @endif
    ...
    <div style="height:30px;background-color: #03a9f4;"></div>
     ...
     <?php 
          ...
          $t_qty=$item['qty']*$item['pack_size']; 

           $total += $t_qty; 

           $l=$item['issue_item']['quantity']*$item['pack_size']; 
          ?>
